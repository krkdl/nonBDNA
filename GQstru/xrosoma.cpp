...
long XROSOMA::read_DNK( const char *pFolder, char *fName )
{
	...
	*(pbody+1) = '\0';
    ...
    noTcx = markTCX_TAG ( );   //move it action into 'LoadXroZones( )'
#endif
...
long XROSOMA::markTCX_TAG ( MUZONE &mz )
{
    char *pStart = Xbody + mz.startPos -1;
    char *pStop = Xbody +  mz.stopPos -1;
    char *pT = Xtag + mz.startPos -1;
    //char *pT = pStart;
    long cnt=0;
    int Route;
    
    for ( char *pXb=pStart; pXb<=pStop; pXb++, pT++ )  {
        if ( (*pXb=='C' && *(pXb-1)=='T') ||
             (*pXb=='G' && *(pXb+1)=='A')    ) {
            SET_TCx_TAG(pT);
            cnt++;
            ...
            if ( GET_RTS_BK0(pT) )
                Route = 0;
            calcRoutStrand( *pXb, mz.Strand, Route, cntTARG );
        }
...
long XROSOMA::calcTCx_OutZ ( )
//   calc Targets out All Zones
{
    char *pStop = Xbody + Xsize -1;
    char *pT=Xtag;
    long cnt=0;
    ...
    for ( char *pXb=Xbody; pXb<=pStop; pXb++, pT++ )  {
        ...
        if ( GET_RTS_BK0(pT) )
                Route = 0;
        Strand = ( *pXb=='C' ) ? '+' : '-';
        calcRoutStrand( *pXb, Strand, Route, cntTARG );
    }
    return cnt;
}
////////////////////////////////////////////////////////////////////////////
