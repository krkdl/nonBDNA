...
int LoadXroZones( const char *pFolder, const char *fSetID )
{
    ...
    while ( 1 )    {
        ...
        noDNK++;
//        printf("\t%02d: chr%d [%ld]=%ld\n",
//               noDNK, XroId, vecDNK[nX].vecMuZon.size(), n);
    }
...
//                     X  Sou Type Start Stop  L  Scor Str  Rept Spac Perm
        ...
        noNucInZ += ( mz.stopPos-mz.startPos+1);
        ...
        if ( mz.spacer==0 )
            continue;
        pStart += mz.repeat;             //first_Spacer
        pStop   = pStart + mz.spacer-1;  //last_Spacer
        for ( char *pX=pStart; pX<=pStop;  pX++ )
            SET_LOOP_TAG(pX);
        
    }
    
    markCornerC( );         // SET_CORN_TAG
    fclose(fiZon);
...
int XROSOMA::  markCornerC( )
{
    ...
    for ( iterZ=vecMuZon.begin(); iterZ!=vecMuZon.end(); iterZ++ )   {
        if ( iterZ->spacer == 0 )
            continue;
        L_corn = iterZ->startPos-1 + iterZ->repeat;
        R_corn = L_corn + iterZ->spacer - 1;           // -1 so as it last_spacer_nuc
        if ( *(Xbody + L_corn) == 'G' )
            ...
            if ( *(Xbody + R_corn) == 'C' )
                ...
                continue;
        if ( GET_CORN_TAG(Xtag+indMut) )    {
//            cntDubl++;
            continue;
        }
        SET_CORN_TAG(Xtag+indMut);
        cntCC++;
    }
