...
//    Using ay case run without RTsegmenys
{
    ...
    for( int nX=0; nX<vecDNK.size(); nX++ )    {
        ...
        vecDNK[nX].nNucInZon = 0;
        vecDNK[nX].nNucOutAllZon = 0;
        vecDNK[nX].nNucOutCurZon = 0;
        ...
        for( pX=vecDNK[nX].Xtag; pX<=pXlast; pX++ ) {
            ...
            vecDNK[nX].nNucOutCurZon++;
            if ( ! GET_MUZTWO_TAG(pX) )
                vecDNK[nX].nNucOutAllZon++; 
        }
...
#if defined (DEBUG_TRACE_RTSEG)
    fprintf(tsld, "\tX%d.creatRTSEGbody [%ld-%ld]\t",
            chrNum, StartSegIndx, StopSegIndx);
    fprintf(tsld, "LngSegm= %ld\tRTsize= %ld (of %ld)\t inMuZon= %ld\n",
            lastPos-firstPos+1, RTsize, RTMemSize, nNucInZon );
#endif
