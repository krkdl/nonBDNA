...
#ifdef RANDOM_TRACE
			fprintf(fRandTrace, "%d\t%d\t%d", nCycl+1,
                    sumXr.nRAND_IN, sumXr.cntLessReal );
			for ( nX=0; nX<NO_HUMAN_XRO; nX++ )	
				if ( muXr[nX].nMut > 0 )
					fprintf(fRandTrace, "\t%d\t%d", muXr[nX].nRAND_IN, muXr[nX].cntLessReal);
			fprintf(fRandTrace, "\n");
#endif
...
void printTableCap( FILE *fRez )
{
    ...
    fprintf(fRez, "sample\tszIN\tmuIN\tszOUall\tmuOUall\tszOUcur\tmuOUcur\tSIGN");
...
//        sumRTsize=0, sumTCXsize=0;
    
    for ( int nX=0; nX<vecDNK.size(); nX++ ) {
//        sumRTsize       += vecDNK[nX].TagSize;      // must be == nNucInZon
//        sumTCXsize      += vecDNK[nX].TCXsize;               //
        
        sumNucIN   += vecDNK[nX].nNucInZon;   // ZONEsize in RT
...
// fprintf(fRez, "sample\tSzIN\tMuIN\tSzOUTall\tMuOUTall\tSzOUTcur\tMuOUTcur\tSIGN");
    fprintf(fRez, "%s\t%ld\t%d\t%ld\t%d\t%ld\t%d\t%.3f",
            SampName.c_str(),
